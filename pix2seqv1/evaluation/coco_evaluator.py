...
class COCOMeanAveragePrecision:
    ...
    def __init__(self, iou_threshold: float = None, verbose=False):
        ...
        """
        # iou_threshold: 如果指定，则只计算 mAP@0.5；如果不指定，则计算 COCO 标准 mAP@[.5:.95]
    ...
    def compute(self, targets_json: dict, predictions_json: List[dict]):
        ...
        coco_eval.evaluate()  # 计算IoU矩阵
        coco_eval.accumulate()  # 计算P-R曲线
        if self.foreground_threshold is None:
            coco_eval.summarize()  # 打印标准表格
    ...
    def _build_coco_eval(self, targets, preds):
        ...
        coco_targets = COCO()  # 初始化
        coco_targets.dataset = targets  # 注入 GT dict
        coco_targets.createIndex()  # 建立索引 (ImageID -> Annotation)
        coco_preds = coco_targets.loadRes(preds)  # 载入预测结果，自动匹配 ImageID
        # 初始化评估器，iouType="bbox" 表示检测任务
